...
###   This script attempts to download the signature file SHA256SUMS.asc from mb8coin.org
...
WORKINGDIR="/tmp/mb8coin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://mb8coin.org/bin/"
VERSIONPREFIX="mb8coin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'mb8coin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the MB8Coin Core binary release signing key installed?"
   fi
