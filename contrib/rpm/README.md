...
The RPM spec file provided here is for MB8Coin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://mb8coin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[MB8Coin git repository](https://github.com/mb8coin/mb8coin) but are not part of
...
Currently only `Source10` is used, for the example `mb8coin.conf` file.
...
in the mb8coin git repository. At present only an SVG version of the MB8Coin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the mb8coin
...
needed when 0.12.1 is released, a proper fix is already in the MB8Coin git
master branch.
...
idea to build MB8Coin Core against a static version of that library compiled
...
The `mb8coin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `mb8coin-server` package. At this point, most RPM based
...
The `mb8coin-server` package should have SELinux support. How to properly do
...
MB8Coin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/mb8coin/). He has been
packaging MB8Coin for Fedora at least since 2012.
...
4. I renamed the `mb8coin` package that contains the Qt GUI to `mb8coin-core` as
that appears to be how the general population refers to it, in contrast to
`mb8coin-xt` or `mb8coin-classic`. I wanted to make sure the general population
