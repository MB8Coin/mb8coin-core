...
MB8Coin Core now uses 'headers-first synchronization'. This means that we first ask peers for block headers and validate those. In a second stage, when the headers have been discovered, we download the blocks. However, as we already know about the whole chain in advance, the blocks can be downloaded in parallel from all available peers.
...
Another change in the 4.0 release is moving away from the mb8coind executable functioning both as a server and as a RPC client. The RPC client functionality ("tell the running mb8coin daemon to do THIS") was split into a separate executable, 'mb8coin-cli'.
...
It has been observed that many of the RPC functions offered by mb8coind are "pure functions", and operate independently of the mb8coind wallet. This included many of the RPC "raw transaction" API functions, such as createrawtransaction.
mb8coin-tx is a newly introduced command line utility designed to enable easy manipulation of MB8Coin transactions. A summary of its operation may be obtained via "mb8coin-tx --help"
...
Starting with Tor version 0.2.7.1 it is possible, through Tor's control socket API, to create and destroy 'ephemeral' hidden services programmatically. MB8Coin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available), MB8Coin Core automatically creates a hidden service to listen on, without manual configuration. MB8Coin Core will also use Tor automatically to connect to other .onion nodes if the control socket can be successfully opened. This will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if MB8Coin Core is listening, and a connection to Tor can be made. It can be configured with the -listenonion, -torcontrol and -torpassword settings. To show verbose debugging information, pass -debug=tor.
...
mb8coin-${VERSION}-arm-linux-gnueabihf.tar.gz: Linux binaries targeting the 32-bit ARMv7-A architecture.

mb8coin-${VERSION}-aarch64-linux-gnu.tar.gz: Linux binaries targeting the 64-bit ARMv8-A architecture.
...
There is no distinction between internal (change) and external keys. HD wallets are incompatible with older versions of MB8Coin Core. Segregated Witness
...
### New MB8Tech RPC Commands

A command to generate a private payment has been added. MB8Coin Core will take care of the negotiation with the MB8Tech nodes and will create and broadcast the transaction. The syntax is:
...
### Management of MB8Tech servers

MB8Tech servers added/removed with the RPC command addanonserver are now stored on the config file when specified by an extra parameter.
