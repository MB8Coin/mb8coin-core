...
UniValue MB8Server::CreateAnonTransaction(string address, CAmount nValue, int nTransactions) {
  ...
  if (encryptionSuccess) {
    ...
    UniValue mb8serverData;
    mb8serverData.setObject();
    mb8serverData.pushKV("anondestination", encryptedAddress);
    mb8serverData.pushKV("public_key", pubKey);
    mb8serverData.pushKV("anonaddress", addrArray);
    mb8serverData.pushKV("anonfee", find_value(serverData, "transaction_fee"));
    mb8serverData.pushKV("min_amount", find_value(serverData, "min_amount"));
    return mb8serverData;
  } else {
    throw runtime_error("Unable to send MB8Tech transaction, please try again.");
  }

}

vector<anonServer> MB8Server::GetAnonServers() {

  vector<anonServer> returnServers;

  if (vAddedAnonServers.size() < 1 && mapMultiArgs["-addanonserver"].size() < 1) {
      throw runtime_error("You must have at least one MB8Tech server added to your conf file or by rpc command.");
  ...
  if (returnServers.size() < 1) {
      throw runtime_error("Please, add some valid MB8Tech server to send private payments.");
      return returnServers;
  }
...
UniValue MB8Server::FindAnonServer(std::vector<anonServer> anonServers, CAmount nValue, int nTransactions) {
  if (anonServers.size() < 1) {
      throw runtime_error("None of your configured MB8Tech nodes are available right now.");
  }
...
UniValue MB8Server::ParseJSONResponse(string readBuffer) {
  try {
      UniValue valRequest;
...
string MB8Server::EncryptAddress(string address, string pubKeyStr, int nTransactions, int nPiece, long nId) {

  address = "{\"n\":\""+address+"\",\"t\":"+std::to_string(GetArg("anon_out_delay",MB8TECH_DEFAULT_OUT_DELAY))+",\"p\":"+std::to_string(nPiece)+",\"o\":"+std::to_string(nTransactions)+",\"u\":"+std::to_string(nId)+"}";
...
int MB8Server::PublicEncrypt(unsigned char* data, int data_len, unsigned char* key, unsigned char* encrypted)
{
...
RSA * MB8Server::CreateRSA(unsigned char * key,int isPublic)
{
    RSA *rsa= NULL;
    BIO *keybio ;
...
bool MB8Server::TestEncryption(string encrypted, UniValue serverData) {
...
UniValue MB8Server::GetServerInfo(std::string server) {
  curl_global_init(CURL_GLOBAL_ALL);
  curl = curl_easy_init();
  string readBuffer;
  ...
  if (curl) {
    ...
    if (res != CURLE_OK) {
      ...
      if (!data_obj.isObject()) {
        ...
        UniValue mb8serverData;
        mb8serverData.setObject();
        mb8serverData.pushKV("min_amount", find_value(data, "min_amount"));
        mb8serverData.pushKV("max_amount", find_value(data, "max_amount"));
        mb8serverData.pushKV("transaction_fee", find_value(data, "transaction_fee"));
        curl_easy_cleanup(curl);
        curl_global_cleanup();
        return mb8serverData;
      }
