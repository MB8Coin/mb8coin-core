...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MB8Coin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MB8Coin
    // Mac: ~/Library/Application Support/MB8Coin
    // Unix: ~/.mb8coin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MB8Coin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MB8Coin";
#else
    // Unix
    return pathRet / ".mb8coin";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", MB8COIN_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No mb8coin.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override mb8coin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", MB8COIN_PID_FILENAME));
