...
bool CFund::IsValidPaymentRequest(CTransaction tx)
{
    ...
    std::string Secret = "I kindly ask to withdraw " + std::to_string(nAmount) + "MB8 from the proposal " + proposal.hash.ToString() + ". Payment request id: " + strDZeel;

    CMB8CoinAddress addr(proposal.Address);
    CKeyID keyID;
    addr.GetKeyID(keyID);
