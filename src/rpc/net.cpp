...
UniValue getaddednodeinfo(const JSONRPCRequest &request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            ...
            "       {\n"
            "         \"address\" : \"192.168.0.201:5556\",  (string) The mb8coin server IP and port we're connected to\n"
...
UniValue addanonserver(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() > 3 ||
        ...
        throw runtime_error(
            ...
            "\nAttempts add or remove a node from the MB8Tech server list.\n"
            "\nArguments:\n"
            "1. \"node\"     (string, required) The node (see getpeerinfo for nodes)\n"
    ...
    if (strCommand == "add")
    {
      if (request.params.size() == 3 && request.params[2].get_str() == "true") {
        ...
        if (it != vAddedAnonServers.end())
          throw JSONRPCError(RPC_CLIENT_NODE_ALREADY_ADDED, "Error: MB8Tech server already added");
        vAddedAnonServers.push_back(strNode);
      }
    }
    else if(strCommand == "remove")
    {
      if (request.params.size() == 3 && request.params[2].get_str() == "true") {
        ...
        if (it == vAddedAnonServers.end())
          throw JSONRPCError(RPC_CLIENT_NODE_NOT_ADDED, "Error: MB8Tech server has not been added.");
        vAddedAnonServers.erase(it);
      }
    }
    return NullUniValue;
...
UniValue getnetworkinfo(const JSONRPCRequest &request)
{
    if (request.fHelp || request.params.size() != 0)
        throw runtime_error(
            ...
            "  \"ntptimeoffset\": xxxxx,                (numeric) the ntptime offset\n"
...
UniValue getstakesubsidy(const JSONRPCRequest &request)
{
    ...
    try {
        ...
        throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "MB8 decode failed");
    }
    ...
    return (uint64_t)GetProofOfStakeReward(pindexBestHeader->nHeight, nCoinAge, 0, pindexBestHeader, true);
}
