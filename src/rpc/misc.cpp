...
UniValue validateaddress(const JSONRPCRequest &request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            "validateaddress \"mb8coinaddress\"\n"
            "\nReturn information about the given mb8coin address.\n"
            "\nArguments:\n"
            "1. \"mb8coinaddress\"     (string, required) The mb8coin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"mb8coinaddress\", (string) The mb8coin address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: MB8Coin address and we have full public key:
        CMB8CoinAddress address(ks);
...
UniValue createmultisig(const JSONRPCRequest &request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are mb8coin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) mb8coin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
    ...
    CMB8CoinAddress address(innerID);
...
UniValue createwitnessaddress(const JSONRPCRequest &request)
{
    ...
    CMB8CoinAddress address(witscriptid);
...
UniValue verifymessage(const JSONRPCRequest &request)
{
    if (request.fHelp || request.params.size() != 3)
        throw runtime_error(
            "verifymessage \"mb8coinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"mb8coinaddress\"  (string, required) The mb8coin address to use for the signature.\n"
...
{
    if (type == 2) {
        address = CMB8CoinAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CMB8CoinAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
bool getAddressesFromRequest(const JSONRPCRequest &request,
    ...
    if (request.params[0].isStr()) {
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CMB8CoinAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
