...
{
    LogPrintf("MB8CoinStaker started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mb8coin-staker");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in MB8CoinStaker: Keypool ran out, please call keypoolrefill before restarting the staking thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;

            //LogPrint("coinstake","Running MB8CoinStaker with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("MB8CoinStaker terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("MB8CoinStaker runtime error: %s\n", e.what());
        return;
    }
}
  ...
  if (nSearchTime > nLastCoinStakeSearchTime)
  {
      ...
      if (wallet.CreateCoinStake(wallet, pblock->nBits, nSearchInterval, nFees, txCoinStake, key))
      {
          ...
          if (txCoinStake.nTime >= pindexBestHeader->GetPastTimeLimit()+1)
          {
              ...
              txCoinStake.nVersion = CTransaction::TXDZEEL_VERSION;
...
bool CheckStake(CBlock* pblock, CWallet& wallet, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        {
            return error("MB8CoinStaker: ProcessNewBlock, block not accepted");
        }
    }
