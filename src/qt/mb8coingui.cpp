...
const std::string MB8CoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString MB8CoinGUI::DEFAULT_WALLET = "~Default";

MB8CoinGUI::MB8CoinGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
...
void MB8CoinGUI::createActions()
{
    QActionGroup *tabGroup = new QActionGroup(this);

    overviewAction = new QAction(platformStyle->SingleColorIcon(":/icons/mb8coin"), tr("&Overview"), this);
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a MB8Coin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and mb8coin: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your MB8Coin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified MB8Coin addresses"));
    ...
    openAction->setStatusTip(tr("Open a mb8coin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible MB8Coin command-line options").arg(tr(PACKAGE_NAME)));
...
void MB8CoinGUI::createMenuBar()
{
    ...
    if(walletFrame)
    {
        ...
        Q_FOREACH(MB8CoinUnits::Unit u, MB8CoinUnits::availableUnits())
        {
            QAction *menuAction = new QAction(QString(MB8CoinUnits::name(u)), this);
...
void MB8CoinGUI::onCurrencySelection(QAction* action)
{
    if (action)
    {
...
void MB8CoinGUI::createToolBars()
{
    if(walletFrame)
    {
...
void MB8CoinGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool MB8CoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool MB8CoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void MB8CoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void MB8CoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void MB8CoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void MB8CoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void MB8CoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void MB8CoinGUI::optionsClicked()
{
...
void MB8CoinGUI::showDebugWindow()
{
...
void MB8CoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void MB8CoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void MB8CoinGUI::openClicked()
{
...
void MB8CoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    topMenu1->setStyleSheet(
...
void MB8CoinGUI::gotoHistoryPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void MB8CoinGUI::gotoReceiveCoinsPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void MB8CoinGUI::gotoRequestPaymentPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void MB8CoinGUI::gotoSendCoinsPage(QString addr)
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void MB8CoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void MB8CoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void MB8CoinGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to MB8Coin network", "", count));
    if(walletFrame){
...
void MB8CoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if(!clientModel)
        return;
...
void MB8CoinGUI::showVotingDialog()
{
...
void MB8CoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("MB8Coin"); // default title
...
void MB8CoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void MB8CoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void MB8CoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(MB8CoinUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void MB8CoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void MB8CoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool MB8CoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool MB8CoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void MB8CoinGUI::setEncryptionStatus(int status)
{
...
void MB8CoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void MB8CoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void MB8CoinGUI::detectShutdown()
{
...
void MB8CoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void MB8CoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void MB8CoinGUI::subscribeToCoreSignals()
{
...
void MB8CoinGUI::unsubscribeFromCoreSignals()
{
    ...
    menu(0)
{
    ...
    QList<MB8CoinUnits::Unit> units = MB8CoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const MB8CoinUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(MB8CoinUnits::name(unit)));
    }
...
{
    setText(MB8CoinUnits::name(newUnits));
}
...
void MB8CoinGUI::replyFinished(QNetworkReply *reply)
{
...
void MB8CoinGUI::replyVotingFinished(QNetworkReply *reply)
{
  ...
  CMB8CoinAddress addr("NMYuCvBiRgvkzjdEBGJHj7rpAnRmfUD6gw");
  CKeyID keyID;
  addr.GetKeyID(keyID);
...
void MB8CoinGUI::getVotingInfo()
{
    ...
    config.setProtocol(QSsl::TlsV1_2);
    request.setSslConfiguration(config);
    request.setUrl(QUrl(QString("https://www.mb8coin.org/voting.") + QString((GetBoolArg("-testnet",false) ? "testnet." : "mainnet.")) + QString("json")));
    request.setHeader(QNetworkRequest::ServerHeader, "application/json");
    reply = manager->get(request);
    connect(manager, SIGNAL(finished(QNetworkReply*)), this,
...
void MB8CoinGUI::updateStakingStatus()
{
    updateWeight();
