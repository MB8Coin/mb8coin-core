...
bool WalletModel::validateAddress(const QString &address)
{
  ...
  CMB8CoinAddress addressParsed(address_str);
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered mb8coin address / amount:
            ...
            CScript scriptPubKey = GetScriptForDestination(CMB8CoinAddress(rcp.isanon ? rcp.destaddress.toStdString() : rcp.address.toStdString()).Get());
        ...
        Q_FOREACH(const CRecipient &rcp, transaction.vecSend)
        {
          ...
          Q_FOREACH(const CWalletTx newTx, transaction.vTransactions)
          {
            ...
            Q_FOREACH(const SendCoinsRecipient &rcp, transaction.getRecipients())
            {
              ...
              else if (!rcp.message.isEmpty()) // Message from normal mb8coin:URI (mb8coin:123...?message=example)
...
{
    QString strAddress = QString::fromStdString(CMB8CoinAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CMB8CoinAddress(address).ToString())].push_back(out);
    }
}
...
{
    CTxDestination dest = CMB8CoinAddress(sAddress).Get();
